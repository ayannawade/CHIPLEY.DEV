...
def main():
	...
	if cd["INPUT DIR"] == "ONLINE":
		...
		if cd["INPUT TYPE"] == "FASTQ":
			...
			cd["INPUT FILES FULL"] = tools.get_other(cd["INPUT DIR"], SUFFIX, cd["EXCLUDE"])
	...
	# Specific cases use specified delimiter for each file
	cd["INPUT FILES DELIM TRIMMED"] = []
	for f in cd["INPUT FILES TRIMMED"]:	
		if "INPUT FILES DELIM" in cd:
			cd["INPUT FILES DELIM TRIMMED"].append(f.rsplit(cd["INPUT FILES DELIM"], 1)[0])
		else:
			cd["INPUT FILES DELIM TRIMMED"] = cd["INPUT FILES TRIMMED"]
	...
	safe_list = ["INPUT FILES FULL", "INPUT FILES TRIMMED", "INPUT FILES DELIM TRIMMED", "SRR SPLIT"]
	for var in cd:
	...
	for i in range(0, len(cd["INPUT FILES FULL"])):
		...
		input_delim_trim_ind = cd["INPUT FILES DELIM TRIMMED"][i]
		cmd_ind = cmd_ind.replace("<INPUT FILES DELIM TRIMMED>", input_delim_trim_ind)
